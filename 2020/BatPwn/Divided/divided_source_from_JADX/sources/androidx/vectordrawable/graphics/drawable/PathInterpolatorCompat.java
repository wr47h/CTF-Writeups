...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        if (numPoints > 0) {
            this.f34mX = new float[numPoints];
            this.f35mY = new float[numPoints];
            float[] position = new float[2];
            for (int i = 0; i < numPoints; i++) {
                pathMeasure.getPosTan((((float) i) * pathLength) / ((float) (numPoints - 1)), position, null);
                this.f34mX[i] = position[0];
                this.f35mY[i] = position[1];
            }
            if (((double) Math.abs(this.f34mX[0])) > 1.0E-5d || ((double) Math.abs(this.f35mY[0])) > 1.0E-5d || ((double) Math.abs(this.f34mX[numPoints - 1] - 1.0f)) > 1.0E-5d || ((double) Math.abs(this.f35mY[numPoints - 1] - 1.0f)) > 1.0E-5d) {
                StringBuilder sb = new StringBuilder();
                ...
                sb.append(" end:");
                sb.append(this.f34mX[numPoints - 1]);
                sb.append(str);
                sb.append(this.f35mY[numPoints - 1]);
    ...
    public float getInterpolation(float t) {
        ...
        int startIndex = 0;
        int endIndex = this.f34mX.length - 1;
        while (endIndex - startIndex > 1) {
            int midIndex = (startIndex + endIndex) / 2;
            if (t < this.f34mX[midIndex]) {
                endIndex = midIndex;
            } else {
                startIndex = midIndex;
            }
